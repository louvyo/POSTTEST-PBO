...
package com.mycompany.posttest_3;
...
class BarangElektronik extends Barang {
    ...
    public BarangElektronik(String kode, String nama, int harga, String jenis) {
...
class BarangNonElektronik extends Barang {
    ...
    public BarangNonElektronik(String kode, String nama, int harga, String bahan) {
        super(kode, nama, harga);
        this.bahan = bahan;
    }
    ...
    static ArrayList<Barang> daftarBarang = new ArrayList<>();
     ...
     static void tambahBarang() {
        ...
        if (jenis.equals("elektronik")) {
            System.out.print("Masukkan jenis elektronik: ");
            String jenisElektronik = scanner.nextLine();
            barang = new BarangElektronik(kode, nama, harga, jenisElektronik);
        } else {
            System.out.print("Masukkan bahan barang non-elektronik: ");
            String bahanNonElektronik = scanner.nextLine();
            barang = new BarangNonElektronik(kode, nama, harga, bahanNonElektronik);
        }
     ...
     static void lihatDaftarBarang() {
        if (daftarBarang.isEmpty()) {
            ...
            System.out.flush();
            System.out.println("======================================================================");
            System.out.println(String.format("%-10s %-20s %-10s %s", "Kode", "Nama", "Harga", "Jenis/Bahan"));
            System.out.println("----------------------------------------------------------------------");
            for (Barang barang : daftarBarang) {
                if (barang instanceof BarangElektronik) {
                    BarangElektronik barangElektronik = (BarangElektronik) barang;
                    System.out.println(String.format("%-10s %-20s %-10s %s", barang.getKode(), barang.getNama(), barang.getHarga(), barangElektronik.getJenis()));
                } else if (barang instanceof BarangNonElektronik) {
                    BarangNonElektronik barangNonElektronik = (BarangNonElektronik) barang;
                    System.out.println(String.format("%-10s %-20s %-10s %s", barang.getKode(), barang.getNama(), barang.getHarga(), barangNonElektronik.getBahan()));
                } else {
